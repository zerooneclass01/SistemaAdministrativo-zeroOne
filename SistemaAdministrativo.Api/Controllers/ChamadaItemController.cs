...
namespace SistemaAdministrativo.Api.Controllers
{
    ...
    public class ChamadaItemController : ControllerBase
    {
        ...
        {
            if (itemId == Guid.Empty) return BadRequest("ID do item inválido.");

            var sucesso = await _itemService.AtualizarPresencaIndividual(itemId, model.Presente, model.Observacao);

            if (!sucesso)
                return BadRequest("Não foi possível atualizar a presença.");

            return Ok("Presença individual atualizada com sucesso!");
        }
        ...
        {
            if (alunoId == Guid.Empty) return BadRequest("ID do aluno inválido.");
